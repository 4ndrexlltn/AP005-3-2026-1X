...
ARCHIVO_CSV = "datos.csv"  # ← ajustado a tu proyecto
...
def generar_grafica(df):
    ...
    try:
        ...
        print(f"[ERROR] No se pudo generar la gráfica: {error}")
...
def inicio():
    try:
        ...
        if df.empty:
            tabla_html = "<p>No hay datos todavía.</p>"
        else:
            tabla_html = df.tail(10).to_html(
                classes="tabla-datos", index=False, border=0
            )
